...
async function testPath(path) {
  ...
  try {
    ...
    try {
      ...
      console.log("Response JSON (truncated):", JSON.stringify(parsed, null, 2).substring(0, 1500));
    } catch(e) {
...
async function start() {
  await testPath('/invoice/all');
  await testPath('/invoice/sent');
  await testPath('/invoice/paid');
  await testPath('/invoice/overdue');
}

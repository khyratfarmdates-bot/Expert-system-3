...
export function handleFirestoreError(error: unknown, operationType: OperationType, path: string | null, auth?: Auth) {
  ...
  // If it's a "unavailable" error, we might want to notify the user differently
  if (errInfo.error.includes('unavailable') || errInfo.error.includes('Could not reach Cloud Firestore')) {
